...
public class RecentsAnimationDeviceState implements DisplayInfoChangeListener, ExclusionListener {
    ...
    RecentsAnimationDeviceState(
        ...
        mIsOneHandedModeSupported =
            SkinApp.isRecentsEnabled() && SystemProperties.getBoolean(SUPPORT_ONE_HANDED_MODE,
                false);
        ...
        if (mIsOneHandedModeSupported) {
            ...
            mIsOneHandedModeEnabled = SkinApp.isRecentsEnabled() && settingsCache.getValue(oneHandedUri);
        ...
        mIsSwipeToNotificationEnabled = SkinApp.isRecentsEnabled() && settingsCache.getValue(swipeBottomNotificationUri);
        lifeCycle.addCloseable(
                () -> settingsCache.unregister(swipeBottomNotificationUri, onChangeListener));

        Uri setupCompleteUri = Settings.Secure.getUriFor(Settings.Secure.USER_SETUP_COMPLETE);
        mIsUserSetupComplete = SkinApp.isRecentsEnabled() && settingsCache.getValue(setupCompleteUri, 0);
        ...
        try {
            mPipIsActive = SkinApp.isRecentsEnabled() && Utilities.ATLEAST_S && ActivityTaskManager.getService().getRootTaskInfo(
