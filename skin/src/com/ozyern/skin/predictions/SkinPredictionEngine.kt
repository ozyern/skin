...
class SkinPredictionEngine(
    ...
    private fun shouldUseWeightedUsageStats(): Boolean {
        if (!prefs2.skinPredictorUseWeightedUsageStats.firstCached()) return false
        ...
        return userProfilesInPredictionOrder()
            ...
            .flatMap { user ->
                try {
                    ...
                    // Skin-Note: Android 17 QPR2 Beta 3 crash when accessing activity list with null pkgName for non-Main user
                    // Ref: https://issuetracker.google.com/issues/547643926
                    Log.e("SkinPredictionEngine", "Failed to get activity list for user $user", e)
                    emptyList()
                }.asSequence()
            }
