...
import com.ozyern.skin.preferences.not
import com.ozyern.skin.preferences.preferenceManager
...
import com.ozyern.skin.ui.preferences.components.HiddenAppsInSearchPreference
...
import com.ozyern.skin.ui.preferences.components.controls.TwoTargetSwitchPreference
import com.ozyern.skin.ui.preferences.components.layout.PreferenceGroup
import com.ozyern.skin.ui.preferences.navigation.SearchProviderPreference
...
fun DrawerSearchPreference(
    ...
    MainSwitchPreference(
        ...
        PreferenceGroup(heading = stringResource(id = R.string.show_search_result_types)) {
            if (searchAlgorithm != SkinSearchAlgorithm.ASI_SEARCH) {
                val canDisable = searchAlgorithm != SkinSearchAlgorithm.APP_SEARCH
            ...
            when (searchAlgorithm) {
                SkinSearchAlgorithm.LOCAL_SEARCH -> {
                    LocalSearchSettings(
                        prefs = prefs,
                        prefs2 = prefs2,
                        context = context,
                ...
                SkinSearchAlgorithm.ASI_SEARCH -> {
...
private fun SearchProvider(
    ...
    val searchAlgorithmEntries = remember {
        sequenceOf(
            ListPreferenceEntry(SkinSearchAlgorithm.APP_SEARCH) { stringResource(R.string.search_algorithm_app_search) },
            ListPreferenceEntry(SkinSearchAlgorithm.LOCAL_SEARCH) { stringResource(R.string.search_algorithm_global_search_on_device) },
            ListPreferenceEntry(SkinSearchAlgorithm.ASI_SEARCH) { stringResource(R.string.search_algorithm_global_search_via_asi) },
        ).filter {
            when (it.value) {
                SkinSearchAlgorithm.ASI_SEARCH -> SkinSearchAlgorithm.isASISearchEnabled(
                    context,
                )
