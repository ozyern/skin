...
fun AppGesturePreference(
        ...
        object : ResultReceiver(Handler(Looper.getMainLooper())) {
            override fun onReceiveResult(resultCode: Int, resultData: Bundle?) {
                if (resultCode == android.app.Activity.RESULT_OK) {
                    val handlerString = resultData?.getString(SkinShortcutActivity.EXTRA_HANDLER)
    ...
    PreferenceTemplate(
        ...
        onClick = {
            mMSDLPlayerWrapper.playToken(MSDLToken.TAP_MEDIUM_EMPHASIS)
            val intent = Intent(context, SkinShortcutActivity::class.java).apply {
                putExtra(SkinShortcutActivity.EXTRA_RESULT_RECEIVER, resultReceiver)
            }
