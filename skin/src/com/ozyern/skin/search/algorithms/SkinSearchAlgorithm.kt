...
import com.ozyern.skin.allapps.views.SearchItemBackground
import com.ozyern.skin.allapps.views.SearchResultView.Companion.EXTRA_QUICK_LAUNCH
import com.ozyern.skin.preferences2.PreferenceManager2
import com.ozyern.skin.preferences2.firstCached
import com.ozyern.skin.search.SkinSearchAdapterProvider
import com.ozyern.skin.search.adapter.START_PAGE
import com.ozyern.skin.search.adapter.SearchAdapterItem
import com.ozyern.skin.search.adapter.SearchTargetCompat
import com.ozyern.skin.search.adapter.SearchTargetCompat.Companion.RESULT_TYPE_APPLICATION
import com.ozyern.skin.search.adapter.SearchTargetCompat.Companion.RESULT_TYPE_SHORTCUT
...
sealed class SkinSearchAlgorithm(
    ...
    protected fun transformSearchResults(results: List<SearchTargetCompat>): List<SearchAdapterItem> {
        val filtered = results
            ...
            .filter { SkinSearchAdapterProvider.viewTypeMap[it.layoutType] != null }
...
//        layoutType == EDUCARD -> transparentBackground
    ...
    companion object {
        ...
        fun isASISearchEnabled(context: Context): Boolean {
            ...
            if (!SkinApp.isRecentsEnabled) return false

            if (!ranCompatibilityCheck) {
                ranCompatibilityCheck = true
                SkinASISearchAlgorithm.checkSearchCompatibility(context)
            }
            return true
        }

        fun create(context: Context): SkinSearchAlgorithm {
            val prefs = PreferenceManager2.getInstance(context)
            val searchAlgorithm = prefs.searchAlgorithm.firstCached()

            return when {
                searchAlgorithm == ASI_SEARCH && isASISearchEnabled(context) -> SkinASISearchAlgorithm(
                    context,
                )

                searchAlgorithm == LOCAL_SEARCH -> SkinLocalSearchAlgorithm(context)

                else -> SkinAppSearchAlgorithm(context)
            }
        }
    }
}
