...
import com.ozyern.skin.launcherNullable
import com.ozyern.skin.util.categorizeAppsWithSystemAndGoogle
...
class SkindeckManager(private val context: Context) {

    // TODO

    private val launcher = context.launcherNullable ?: SkinLauncher.instance?.launcher

    suspend fun enableSkindeck(
        onProgress: ((String) -> Unit)? = null,
    ) = withContext(Dispatchers.IO) {
        val completionDeferred = CompletableDeferred<Unit>()

        if (!backupExists("bk")) createBackup("bk")
        if (backupExists("skindeck")) {
            onProgress?.invoke("Restoring previous layout...")
            restoreBackup("skindeck")
    ...
    suspend fun disableSkindeck() = withContext(Dispatchers.IO) {
        if (backupExists("bk")) {
            createBackup("skindeck")
            restoreBackup("bk")
        }
    }
    ...
    }.onFailure { Log.e("SkindeckManager", "Failed to create backup: $suffix", it) }
    ...
    private fun restoreBackup(suffix: String) = runCatching {
        ...
        postRestoreActions()
    }.onFailure { Log.e("SkindeckManager", "Failed to restore backup: $suffix", it) }
