...
import com.ozyern.skin.SkinApp.Companion.showQuickstepWarningIfNecessary
import com.ozyern.skin.compat.SkinQuickstepCompat
import com.ozyern.skin.data.AppDatabase
import com.ozyern.skin.data.wallpaper.service.WallpaperService
import com.ozyern.skin.gestures.GestureController
import com.ozyern.skin.gestures.VerticalSwipeTouchController
import com.ozyern.skin.gestures.config.GestureHandlerConfig
import com.ozyern.skin.gestures.ui.SkinShortcutActivity
import com.ozyern.skin.nexuslauncher.OverlayCallbackImpl
...
import com.ozyern.skin.root.RootHelperManager
import com.ozyern.skin.root.RootNotAvailableException
import com.ozyern.skin.theme.ThemeProvider
import com.ozyern.skin.ui.popup.LauncherOptionsPopup
import com.ozyern.skin.ui.popup.SkinShortcut
import com.ozyern.skin.util.getThemedIconPacksInstalled
import com.ozyern.skin.util.unsafeLazy
import com.ozyern.skin.views.SkinFloatingSurfaceView
...
class SkinLauncher : QuickstepLauncher() {
    ...
    private val statusBarClockListener = object : StateManager.StateListener<LauncherState> {
        override fun onStateTransitionStart(toState: LauncherState) {
            when (toState) {
                ...
                is AllAppsState,
                -> {
                    SkinApp.instance.restoreClockInStatusBar()
                }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        layoutInflater.factory2 = SkinLayoutFactory(this)
        ...
        if (prefs.autoLaunchRoot.get()) {
            lifecycleScope.launch {
                try {
                    RootHelperManager.INSTANCE.get(this@SkinLauncher)
    ...
    override fun onNewIntent(intent: Intent?) {
        if (intent != null && intent.action == SkinShortcutActivity.START_ACTION) {
            val handlerString = intent.getStringExtra(SkinShortcutActivity.EXTRA_HANDLER)
    ...
    override fun getAllAppsItemLongClickListener(): View.OnLongClickListener {
        return View.OnLongClickListener { view ->
            if (view is FolderIcon && view.mInfo.id != ItemInfo.NO_ID) {
                SkinShortcut.showAppDrawerFolderPopup(this, view)
            } else {
    ...
    override fun getSupportedShortcuts(container: Int): Stream<SystemShortcut.Factory<*>> = Stream.concat(
        ...
        Stream.concat(
            Stream.of(SkinShortcut.UNINSTALL, SkinShortcut.CUSTOMIZE, SkinShortcut.OPEN_IN_STORE),
            if (SkinApp.isRecentsEnabled) Stream.of(SkinShortcut.PAUSE_APPS) else Stream.empty(),
        ),
    )
    ...
    override fun handleGestureContract(intent: Intent) {
        if (!SkinApp.isRecentsEnabled && prefs.enableGnc.get()) {
            ...
            if (gnc != null) {
                ...
                SkinFloatingSurfaceView.show(this, gnc)
            }
        }
    }
    ...
    override fun makeDefaultActivityOptions(splashScreenStyle: Int): ActivityOptionsWrapper {
        ...
        val options = if (Utilities.ATLEAST_Q) {
            SkinQuickstepCompat.activityOptionsCompat.makeCustomAnimation(
                this,
                0,
                0,
    ...
    private fun restartIfPending() {
        when {
            sRestartFlags and FLAG_RESTART != 0 -> skinApp.restart(false)
    ...
    companion object {
        ...
        val instance get() = SkinApp.launcher
    }
}

val Context.launcher: SkinLauncher
    get() = BaseActivity.fromContext(this)

val Context.launcherNullable: SkinLauncher? get() = try {
    launcher
